...
const apiCanaryBlueprint = async function () {
    ...
    const dataset = body.data.find(ds => ds.attributes.layer.length > 0);
    if (!dataset) {
        throw new Error('No usable dataset.');
    }
    const datasetId = dataset.id;
    const layerId = dataset.attributes.layer[0].id;

    // Change needed request options
    requestOptions.path = "/v1/dataset/" + datasetId + "/layer/" + layerId;
